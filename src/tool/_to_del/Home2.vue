...
<div class="text-left mt-5 col-10 offset-1 col-xl-10 offset-xl-1">
  ...
  <div v-if="$store.state.beAssessment">
    <!-- <h5>{{$store.state.company_name}}</h5> -->
    <!-- {{$store.state.beAssessment.goal_stats}} -->
    ...
    <div class="row mt-4 p-2">
      <table class="table table-sm ">
        ...
        <tbody>
          <tr @click="goto(`be/${item.code}`)" class="be-hover-home" v-for="item in $store.state.beAssessment.goal_stats">
            <td>{{item.code}}</td>
            <td>{{$store.state.BElayout[item.code].title}}</td>
...
<script>
import beSankey from '@/tool/components/beSankey.vue'
import ppSankey from '@/tool/components/ppSankey.vue'
